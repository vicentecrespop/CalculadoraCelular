import React, { Component } from 'react'
import './Calculator.css'

import Button from '../components/Button'
import Display from '../components/Display'


const initialState = {
    displayValue: '0',
    clearDisplay: false,
    operation: null,
    values: [0, 0],
    current: 0,
    newOperation: true
}


export default class Calculator extends Component {

    state = { ...initialState }

    constructor(props) {
        super(props)

        this.clearMemory = this.clearMemory.bind(this)
        this.setOperation = this.setOperation.bind(this)
        this.addDigit = this.addDigit.bind(this)
        this.setNegative = this.setNegative.bind(this)
    }


    clearMemory() {
        this.setState({ ...initialState })
    }

    setNegative() {
        const current = this.state.current
        const values = [...this.state.values]
        const op = this.state.operation
        let operation = op === '*' || op === '/' ? op : '+'
        values[1] = values[1] === 0 && (op === '*' || op === '/') ? 1 : values[1]
        if (current === 1) {
            values[0] = this.solveOperation(values[0], values[1], operation)
            values[1] = 0
        }
        operation = this.state.newOperation ? '+' : operation
        const displayValue = '-'
        this.setState({
            values,
            operation,
            current: 1,
            displayValue,
            clearDisplay: false,
            newOperation: false
        })
    }

    solveOperation(n1, n2, operation) {
        switch(operation) {
            case '*': return n1 * n2
            case '/': return n1 / n2
            case '+': return n1 + n2
            case '-': return n1 - n2
            default: console.log('Erro!! Operação nao existe.')
        }
    }


    setOperation(operation) {
        if (this.state.current === 0) {
            this.setState({ operation, current: 1, clearDisplay: true, newOperation: false })
        } else {
            const equals = operation === '='
            const currentOperation = this.state.operation

            const values = [...this.state.values]
            values[1] = values[1] === 0 && (operation === '*' || operation === '/') ? 1 : values[1]
            try {
                values[0] = this.solveOperation(values[0], values[1], currentOperation)
                if (isNaN(values[0]) || !isFinite(values[0])) {
                    this.clearMemory()
                    return
                }
            } catch (e) {
                values[0] = this.state.values[0]
            }
            values[1] = 0

            this.setState({
                displayValue: values[0],
                operation: equals ? null : operation,
                current: equals ? 0 : 1,
                clearDisplay: !equals,
                values,
                newOperation: false
            })
        }
    }


    addDigit(n) {
        if (n === '.' && this.state.displayValue.includes('.')) {
            return
        }

        const clearDisplay = this.state.displayValue === '0'
            || this.state.clearDisplay
        const currentValue = clearDisplay ? '' : this.state.displayValue
        const displayValue = currentValue + n
        this.setState({ displayValue, clearDisplay: false, newOperation: true })

        if (n !== '.') {
            const i = this.state.current
            const newValue = parseFloat(displayValue)
            const values = [...this.state.values]
            values[i] = newValue
            this.setState({ values })
            console.log(values)
        }
    }

    render() {
        return (
            <div className='calculator'>
                <Display value={this.state.displayValue} />
                <Button label="C" triple click={this.clearMemory} />
                <Button label="/" operation click={this.setOperation} />
                <Button label="7" click={this.addDigit} />
                <Button label="8" click={this.addDigit} />
                <Button label="9" click={this.addDigit} />
                <Button label="*" operation click={this.setOperation} />
                <Button label="4" click={this.addDigit} />
                <Button label="5" click={this.addDigit} />
                <Button label="6" click={this.addDigit} />
                <Button label="+" operation click={this.setOperation} />
                <Button label="1" click={this.addDigit} />
                <Button label="2" click={this.addDigit} />
                <Button label="3" click={this.addDigit} />
                <Button label="-" operation click={this.setNegative} />
                <Button label="0" double click={this.addDigit} />
                <Button label="." click={this.addDigit} />
                <Button label="=" equal operation click={this.setOperation} />
            </div>
        )
    }
}